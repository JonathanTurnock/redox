use mlua::{IntoLua, Lua, Result, Table, UserData, UserDataMethods, UserDataRef, Value};
use sqlx::{Column, Row, SqlitePool};
use tracing::{instrument};

#[instrument(name = "sqlite.query")]
async fn query(lua: Lua, this: UserDataRef<_SqliteConnection>, query: String) -> Result<Vec<Table>> {
...
impl UserData for _SqliteConnection {
    fn add_methods<'lua, M: UserDataMethods<Self>>(methods: &mut M) {
        methods.add_async_method("query", query);
    }
}
